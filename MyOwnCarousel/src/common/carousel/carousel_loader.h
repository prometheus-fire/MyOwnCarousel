...
struct carousel_loader {
	...
	static void push_cameraman(NSVGpath* npath, float rd, std::vector<cameraman>& vc) {
		...
		float deltay = 1.; // this quantity gets added to the height of all cameramen
		so.frame[3] = glm::vec4(npath->pts[0], r()->ter().y(npath->pts[0], npath->pts[1]) + deltay, npath->pts[1], 1.0);
		vc.push_back(so);
	}
