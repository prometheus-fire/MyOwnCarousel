...
/*
This function returns the closest point on the
track to p.
*/
glm::vec3 getClosestPoint(const glm::vec3& p, const track& t) {
	glm::vec3 closestPoint;
	float minDistance = std::numeric_limits<float>::max();

	for (size_t j = 0; j < 2; j++)
	for (size_t i = 0; i < t.curbs[j].size(); ++i) {
		glm::vec3 start = t.curbs[j][i];
		glm::vec3 end = t.curbs[j][(i + 1) % t.curbs[j].size()];
		glm::vec3 segment = end - start;

		// Proiezione scalare del punto p sul segmento
		float t_proj = glm::dot(p - start, segment) / glm::dot(segment, segment);
		t_proj = glm::clamp(t_proj, 0.0f, 1.0f); // Assicura che il punto sia sul segmento

		glm::vec3 projected = start + t_proj * segment;
		float distance = glm::length(p - projected);
...
struct  terrain {
	...
	/*
		I changed hf to use at so that it throws an exception if accessing
		height_field out of range. This is used inside of the normal function
		to easily catch this kind of error.
	*/
	float  hf(const unsigned int i, const unsigned int j) const {
		//return height_field[ (size_pix[0]-1-i) * size_pix[0] + j]/50.f;
		return height_field.at((size_pix[0] - 1 - i) * size_pix[0] + j) / 50.f;
	}
	...
	/*
		Given a 3d point this function returns the normal of its orthogonal projection on the terrain.
		(orthogonal projection on the y direction)
	*/
	glm::vec3 normal(glm::vec3 p_in) {
		float eps = 0.01;

		glm::vec3 px_0 = p_in + glm::vec3(eps, 0, 0);
		glm::vec3 pz_0 = p_in + glm::vec3(0, 0, eps);
		...
		glm::vec3 result = glm::normalize(glm::cross(pz - p0, px - p0));
		return result;
	}
...
public:
	...
	* */
	...
	/*
		This function returns the current day time as a float, where
		0. = 1. = midnight and 0.5 = midday.
		This is used inside of the skybox shader to calculate the sky's color.
	*/
	float getDayTime() {
		int cs = clock() - clock_start;
		int day_ms = 3600000 * 24;
		int daytime = (this->sim_time + cs * sim_time_ratio) % (day_ms);
		return float(daytime) / float(day_ms);
	}
	...
	void update() {
		...
		/*
			I modified the car frame update code so that the
			current frame is interpolated instead of being set directly
			equal. This solves the flickering problem of the moving car
			animation.
		*/

		float elapsed_sec = float(clock() - clock_start) / CLOCKS_PER_SEC;

		for (size_t i = 0; i < _cars.size(); ++i) {
			const auto& path = carpaths[_cars[i].id_path];
			float frame_f = fmod(elapsed_sec * 30.f + _cars[i].delta_i, float(path.frames.size()));
			int frame_i = int(frame_f);
			int next_i = (frame_i + 1) % path.frames.size();
			float alpha = frame_f - frame_i;

			glm::mat4 a = path.frames[frame_i];
			glm::mat4 b = path.frames[next_i];

			glm::mat4 interp_frame = a * (1. - alpha) + b * alpha;

			_cars[i].frame = interp_frame;
		}
